...
import { createPublicBooking, fetchPublicBookingOptions, type PublicConsultType } from "@/lib/publicBooking";
import { cn, toast } from "@/lib/utils";
import { fday, ftime } from "@/lib/api";

function fmtTime(ts: string | number) {
  return ftime(typeof ts === "number" ? new Date(ts).toISOString() : ts);
}
function fmtDay(ts: string | number) {
  return fday(typeof ts === "number" ? new Date(ts).toISOString() : ts);
}
  ...
  for (let h = 9; h < 17; h++) {
    for (const m of [0, 30]) {
      ...
      const taken = takenSlots.some(
        (iso) => Math.abs(new Date(iso).getTime() - x.getTime()) < 30 * 60 * 1000
      );
...
function confirmPreview(name: string, typeName: string, at: number, mode: string, office: string) {
  ...
  return `Hi ${fn}, your ${typeName.toLowerCase()} with Nanak Migration is confirmed for ${fmtDay(at)} at ${fmtTime(at)} (${mode}, looked after by our ${office} office).`;
}

type Props = {
  /** Compact card for admin preview; full page chrome handled by parent. */
  className?: string;
};
...
export default function PublicBookingWizard({ className }: Props) {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: fetchPublicBookingOptions,
    staleTime: 60_000,
    retry: 1,
  });
  ...
  const takenSlots = data?.takenSlots ?? [];
  ...
  const mut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["public-booking-options"] });
      setLastAt(slot);
      setLastType(t);
      setStep(4);
    ...
    <div className={cn("cw", className)}>
      <div className="cw-left">
        <div className="relative z-[1] border-b border-white/16 pb-3">
          ...
          <div className="mt-1 font-mono text-[9.5px] uppercase tracking-[0.09em] text-gold">
        ...
        {t ? (
          <>
            ...
            <div className="relative z-[1] text-xs text-white/85">
      ...
      <div className="cw-right">
        {isLoading && (
          <div className="py-16 text-center text-sm text-muted">Loading available times…</div>
        )}
        {isError && !isLoading && (
        ...
        {!isLoading && step === 1 && (
          <>
            <div className="mb-3.5 text-[16.5px] font-bold tracking-tight text-navy">Select a consultation</div>
            <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
              {types.map((x) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-start gap-1.5 text-[13px] font-bold leading-snug text-navy">
                    {x.name}
                    {x.id === "pr" && (
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <span
                      className={cn(
                        "rounded-md px-2 py-1 font-mono text-[13px] font-semibold",
                        x.fee ? "bg-surface text-navy" : "bg-green-50 text-ok"
                      )}
                    >
                      {x.fee ? `$${x.fee}` : "Free"}
                    </span>
        ...
        {!isLoading && step === 2 && (
          <>
            <button
              type="button"
              className="mb-2.5 rounded-full border border-line px-3 py-1 text-xs text-muted hover:border-navy hover:text-navy"
              onClick={() => {
                setStep(1);
                setSlot(null);
              }}
            ...
            <div className="mb-3.5 text-[16.5px] font-bold tracking-tight text-navy">Pick a date & time</div>
            <div className="grid gap-5 lg:grid-cols-[1fr_156px]">
              <div>
                <div className="mb-2 flex items-center justify-between text-sm font-bold text-navy">
                  <button
                    type="button"
                    className="h-7 w-7 rounded-full border border-line disabled:opacity-30"
                    disabled={monthOff <= 0}
                    onClick={() => {
                      setMonthOff((v) => v - 1);
                      setDay(null);
              ...
              <div className="flex max-h-[310px] flex-col gap-1.5 overflow-y-auto">
                {day ? (
                  <>
                    <div className="text-[11px] font-bold uppercase tracking-wide text-muted">{fmtDay(day.getTime())}</div>
                    {slots.length ? (
                      slots.map((sl) => (
                        <button
                          key={sl.ts}
        ...
        {!isLoading && step === 3 && (
          <>
            ...
            <div className="mb-3.5 text-[16.5px] font-bold tracking-tight text-navy">Your details</div>
            {/* Honeypot */}
            <input
              type="text"
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <label className="block text-xs font-semibold text-muted">
                Full name *
                <input
                  className="mt-1.5 w-full rounded-[9px] border-[1.5px] border-line px-3 py-2.5 text-[13.5px] text-ink outline-none focus:border-navy focus:ring-4 focus:ring-navy/10"
                  value={name}
            ...
            <div className="mt-2.5 grid grid-cols-2 gap-2.5">
              <label className="block text-xs font-semibold text-muted">
                Phone or video?
                <select
                  className="mt-1.5 w-full rounded-[9px] border-[1.5px] border-line px-3 py-2.5 text-[13.5px] text-ink"
                  value={mode}
                  onChange={(e) => setMode(e.target.value as "Video" | "Phone")}
                >
                  <option>Video</option>
                  <option>Phone</option>
                </select>
            ...
            <label className="mt-2.5 block text-xs font-semibold text-muted">
              ...
              <input
                className="mt-1.5 w-full rounded-[9px] border-[1.5px] border-line px-3 py-2.5 text-[13.5px] text-ink"
                placeholder="e.g. 485 ending in 3 months"
                value={topic}
            ...
            <label className="mt-3.5 flex items-start gap-2 text-xs font-normal leading-relaxed text-ink">
              ...
              I consent to a VEVO check of my visa status before the consultation, and I’ll have my passport, visa
              grant letter and CV ready. General information only — not migration advice.
            </label>
        ...
        {!isLoading && step === 4 && lastAt && lastType && (
          <>
            ...
            <label className="mt-3.5 block text-xs font-semibold text-muted">Confirmation</label>
